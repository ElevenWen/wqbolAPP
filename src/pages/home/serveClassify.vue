<template>
  <div id="serveClassify" :class="{'hidden':isShow}" >
    ...
    <div class="popularity_return">
        ...
        <img  class="search-head"  :class="{'isShow':!flexnav}" src="../../assets/images/tabBar/search@3x.png"  @click="sous">
    </div>
    <div class="serveClassify">
      <div class="search_box" v-show="!flexnav">
        <label class="search"><img src="../../assets/images/tabBar/search@3x.png" ></label>
          <input type="text" @click="sous" value="请输入关键字" class="search_in"/>
      </div>
...
<!-- tab切换 -->
   ...
   <!-- 排序 -->
      <div ref="sort"  class="sort" >
        <div class="sort_left" @click="sort_ZH">{{theSorting}}<img src="../../assets/images/businessQuery/down_arrow@3x.png"></div>
      </div>
      <div class="sort_ZH" v-if="isShow" @click='isShow = false'>
        <ul>
          <li @click="sort_Px(1)">按销量排序</li>
          <li @click="sort_Px(2)">价格从高到低</li>
          <li @click="sort_Px(3)">价格从低到高</li>
        </ul>
      </div>
    <!-- /排序 -->
        ...
        <li v-for="data in threeList" :key="data.id" @click="goProduct(data.Id,data.Type)" class="promotion_list"   >
          ...
          <div class="promotion_particulars">
            ...
            <div class="promotion_money">
              ...
              <del class="abolish">{{data.oldPrice  | formatPrice |currency}}<b>{{data.Price  | formatPriceX}}</b><label></label></del> 
...
export default {
  ...
  methods: {
    ...
    sort_Px(val) {
      ...
      this.theSorting =
        val === 1 ? "按销量排序" : val === 2 ? "价格从高到低" : "价格从低到高";
    },
    loadMore() {
      this.pageIndex++;
      let ID = this.datas[this.selected].Id;
