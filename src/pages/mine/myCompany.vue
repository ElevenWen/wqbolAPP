    ...
    <!-- 选择公司 -->
		<div id="companyDetailWrap">
			...
			<ul class="companyWrap"  v-show=" !showBank && !showCompany "> 
			  <li v-for="(items ,index) in  companyList" :key="index">
			  <span class="spanIndex" @click="checkComanyFn(items,index)">{{items.Name}}</span>
			  	<!--<i :class="{cIcon:index==check,'':index!=check}"></i>-->
			  	<!-- {{items}} -->
			  	<span class="company_amount" v-if="items.OrderCount != 0 ">{{items.OrderCount}}</span>
				<span 
					:class="items.IsDefault ? 'defaultC' : 'setDefault'" 
					@click="clickDefault(items,index)"
					v-if="items.ReviewStatus == 1"
				>{{ items.IsDefault ? "默认" : "设为默认"}}</span>
			  </li>
			</ul>
...
//			vm.request_data_goCompany();  
	...
	methods:{
		clickDefault(val,index){
			let params = {
				params:{
					Id :val.Id
				}
			}
			getd.SetDefault(params).then((res) => {
				this.companyList.forEach((val,i) => {
					if(index == i){
						val.IsDefault = true
					}else{
						val.IsDefault = false
