<template>
	<div id="orderServering">
		...
		<div class="swiperWrap" v-if="productArr.length != 0">
			<mt-swipe :show-indicators="false" :auto="5000" >
				<mt-swipe-item v-for="(items,index) in productArr" :key="index">
					<div class="processWrap" @click.stop.prevent="orderSchedule">
						<div class="processCompany">{{items.ProductName}}-{{items.CompanyName}}</div>
						<div :class="items.processArr.TaskState == 0 ? 'processDetailWrap1' : 'processDetailWrap'">
							<div class="processTime">
								<div>{{items.processArr.year}}</div>
								<div>{{items.processArr.hours}}</div>
							</div>
							<div class="processIcon"><div></div></div>
							<div class="processDetail">
								<div>{{items.processArr.status}}</div>
								<div>第{{items.processArr.Sort}}步  {{items.processArr.Name}}（预估{{items.processArr.WorkHour}}）</div>
							</div>
		...
		methods:{
			...
			requestProcess(){
				...
				getData.GetProductProcessById(params).then((res) => {
					...
					this.productArr.forEach((items,i) => {
						if(items.SPCopyListModels.length != 0){
							// 当第一步为未处理时  则代表物流进度没开始处理 则显示第一步
							if(items.SPCopyListModels[0].TaskState == 0){
								items.processArr = items.SPCopyListModels[0]
							}else if(items.SPCopyListModels[items.SPCopyListModels.length - 1].TaskState == 2){
								// 当最后一步为已完成时  则显示已完成
								items.processArr = items.SPCopyListModels[items.SPCopyListModels.length - 1];
							}else{
								let val = items.SPCopyListModels
								for(let i=0;i<val.length;i++){
									if(val[i].TaskState == 1 ){
										items.processArr = val[i];
										break;
									}else if(val[i].TaskState == 2 ){
										if(val[i+1].TaskState == 0){
											items.processArr = val[i+1];
											break;
										}
									}
								}
							...
							if(items.processArr.TaskState == 0){
								items.processArr.status = "未开始"
							}else if(items.processArr.TaskState == 1){
								items.processArr.status = "处理中";
								let currentTime1 = items.processArr.HandleTime.replace(/\D/gi, "");
								items.processArr.year = util.formatDate(currentTime1,"yyyy-MM-dd");
								items.processArr.hours = util.formatDate(currentTime1,"hh:mm:ss");
							}else if(items.processArr.TaskState == 2){
								items.processArr.status = "已完成";
								let currentTime2 = items.processArr.EndTime.replace(/\D/gi, "");
								items.processArr.year = util.formatDate(currentTime2,"yyyy-MM-dd");
								items.processArr.hours = util.formatDate(currentTime2,"hh:mm:ss");
							}
...
#orderServering   ul.server li:nth-child(1){
     width: 60%; 
	 .bg-image("../../assets/images/mine/myorder/contactserver");
