	...
	<div id="orderDetail">
		...
		<div class="toDetail" v-show="toDetail">
			<div class="wrap">
                ...
                <!-- 服务进度 -->
                <!-- <div class="processWrap" v-if="item.ProcessingState == 'InProcess' || item.ProcessingState == 'Completed' || item.ProcessingState == 'Evaluate'">
                  <div class="processDetail">
                    <span></span><span>执照审核</span>
                  </div>
                  <div class="processTime">2018-08-06 &nbsp; 11:03:55</div>
                </div> -->
				    	...
				    	<div class="orderStatus" v-if="item.ProcessingState == 'Prepay'">
				    		<span @click="toPayMoney(item)">立即支付</span>
							<span @click="toHelpedPay(arr[index].OrderNumber,item,arr[index].Amount)">找人代付</span>						
			 

				    		<span @click="cancelOrder(arr,index)">取消订单</span>
				    	</div>
				    	<div class="orderStatus" v-else-if="item.ProcessingState == 'InProcess'">
				    		<span @click="buyAgain(arr,index)">再次购买</span>
                <span @click="seeProcess(arr[index])">查看进度</span>
				    	</div>
				    	<div class="orderStatus" v-else-if="item.ProcessingState == 'Canceled'">
				    		<span @click="buyAgain(arr,index)">再次购买</span>
				    		<span @click="deleteOrder(arr[index],index)">删除订单</span>
				    	</div>
				    	<div class="orderStatus" v-else-if="item.ProcessingState == 'Completed'">
				    		<span @click="buyAgain(arr,index)">再次购买</span>
				    		<span @click="toEvaluation(item,index)" v-if='item.OrderStatus !=="已完成"'>去评价</span>
				    	</div>
				    	<div class="orderStatus" v-else-if="item.ProcessingState == 'Evaluate'">
				    		<span @click="buyAgain(arr,index)">再次购买</span>
				    		<span @click="toEvaluation(item,index)" v-if='item.OrderStatus !=="已完成"'>去评价</span>
				    	</div>
					</ul>
				  		...
				  		<div class="wrap" v-if="newArr1.length > 0">
						    <div class="orderOne">
						    	<div @click="toOrderDetail(newArr1[index])">
							    	<div class="waitPay"><span>待支付</span></div>
						    	...
						    	<div class="orderStatus">
									<span @click="toPayMoney(item)">立即支付</span>
						    		<span @click="toHelpedPay(newArr1[index].OrderNumber,item,newArr1[index].Amount)">找人代付</span>
									<span @click.prevent="cancelOrder(newArr1,index)">取消订单</span>
						    	</div>
						    </div>
			    		</div>
					</ul>
				  ...
				  <mt-tab-container-item id="3">
					   	...
					   	<div class="wrap"  v-if="newArr2.length > 0"> 
							<div class="orderOne" >
								<div @click="toOrderDetail(newArr2[index])">
							    	<div class="waitPay"><span>办理中</span></div>
                    <!-- 服务进度 -->
                    <!-- <div class="processWrap">
                      ...
                      <div class="processTime">2018-08-06 &nbsp; 11:03:55</div>
							    	...
							    	<li v-for="(detail,index) in (newArr2[index].OrderDetails)" :key="index">
								    	<div class="orderDetail">
								    		<div class="orderDetailImg"> 
								    			<img :src="detail.ThumbImgURL" class="orderImg">
								    		</div>
						    	...
						    	<div class="orderStatus">
						    		<span @click="buyAgain(newArr2,index)">再次购买</span>
                    <span @click="seeProcess(item)">查看进度</span>
						    	</div>
						    </div>
					   	</div>
					</ul>
				  		...
				  		<div v-if="newArr4.length > 0">
				  			<div class="orderOne" >
				  				<div @click="toOrderDetail(newArr4[index])">
							    	<div class="waitPay"><span>已完成</span></div>
                    <!-- 服务进度 -->
                    <!-- <div class="processWrap">
                      <div class="processDetail">
                        <span></span><span>执照审核</span>
                      ...
                      <div class="processTime">2018-08-06 &nbsp; 11:03:55</div>
						    	...
						    	<div class="orderStatus">
						    		<span @click="buyAgain(newArr4,index)">再次购买</span>
						    		<span @click="toEvaluation(item,index)" v-if='item.OrderStatus !=="已完成"'>去评价</span>
                    <span @click="seeProcess(item)">查看进度</span>
						    	</div>
						    </div>
				  ...
				  <mt-tab-container-item id="5">
					<ul v-for="(item,index) in newArr3">
						<div  v-if="newArr3.length > 0">
							<div class="orderOne">
							    <div class="waitPay"><i class="delete" @click.stop.prevent="deleteOrder(item,index)"></i><span class="hadCancel">已取消</span></div>
						    	...
						    	<div class="orderStatus">
						    		<span @click="buyAgain(newArr3,index)">再次购买</span>
						    		<span @click.stop.prevent="deleteOrder(item,index)">删除订单</span>
						    	</div>
...
.orderImg {
  ...
  margin: 0.32rem 0 0 0.32432432432432434rem;
}
.orderDetail {
