...
export default {
	...
	created(){   
		...
		this.REQUEST_DATA_MYORDER_ONLOGIN(params).then(()=>{
			console.log(this.onlogin_data,this.onlogin_data.CustomerId)
		})
	},
	computed:{
	...
	methods:{
		handleWX(self){
			Vue.prototype.WXshare(
				...
				`https://host.wqbol.com/helpFriendPay?orderNum=${this.$route.query.orderNum}&actPrice=${this.$route.query.actPrice}&flag=help&customerId=${this.onlogin_data.CustomerId}`
			);
		...
		//    暂时跳转联动
			// this.$router.push({path:'/helpFriendPay',query:{'orderNum':this.$route.query.orderNum,'customerId':this.onlogin_data.CustomerId}})
			//原生判断 
			if(tool.is_weixn()){ 
					//微信分享
					this.handleWX(this);
		...
		qqBtn(val){
			...
			switch(val){
				...
				var qrcode = new QRCode(document.getElementById("QR_code"), { 
				    ...
				    text: `https://host.wqbol.com/helpFriendPay?orderNum=${this.$route.query.orderNum}&actPrice=${this.$route.query.actPrice}&flag=help&customerId=${this.onlogin_data.CustomerId}`,//环境					
				    width:sizeS,
				    height: sizeS,
				    colorDark : "#000000",   
				    colorLight : "#ffffff",
