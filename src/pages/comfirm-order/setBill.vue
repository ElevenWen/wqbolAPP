<template>
	<div id="setBill">
		<div class="popularity_return">  
           ...
           <span class="detail" @click="billDetail()">发票须知</span>
		</div>
		
		<div class="billTypeWrap">
			<div  class="title"><i class="bIcon"></i><span>发票类型</span></div>
			<div class="billType">
				<span v-for=" (value,index)  in  billType" @click="changeTab(index,value)" :class=" {'active':index == showIndex}">{{value}}</span>
			</div>
			<div  class="other">
			...
			<div class="generalWtap"  v-show="0 == showIndex">
				<ul class="general">
					   <form @submit.prevent="submitFn0">  
							<li class="items"  >
								<span>公司名称</span>
								<input placeholder="请输入" name="Name" v-model="inputtext0.Name"   @blur="verifyC(inputtext0.Name)"/>
								<i class="gIcon"@touchend.stop.prevent ="goCompany('0',inputtext0,'goCompany')"></i>
							</li>
							<li class="items"  >
								<span>税号</span>
								<input placeholder="请输入" name="TaxIdNumber" v-model="inputtext0.TaxIdNumber"  @blur="verifyT(inputtext0.TaxIdNumber)" />
							</li>
			...
			<div class="generalWtap"  v-show="1 == showIndex">
				<ul class="general">
					   <form @submit.prevent="submitFn1">  
							<li class="items" >
								<span>公司名称</span>
								<input placeholder="请输入" name="Name" v-model="inputtext1.Name"  @blur="verifyC(inputtext1.Name)" />
								<i class="gIcon" @touchend.stop.prevent="goCompany('1',inputtext1,'goCompany')"></i>
							</li>
							<li class="items"  >
								<span>税号</span>
								<input placeholder="请输入" name="TaxIdNumber" v-model="inputtext1.TaxIdNumber"  @blur="verifyT(inputtext1.TaxIdNumber)" />
							</li>
							<li class="items" >
								<span>地址</span>
								<input placeholder="请输入" name="Address" v-model="inputtext1.Address"  @blur="verifyA(inputtext1.Address)"/>  
							</li>
							<li class="items" >
								<span>电话</span>
								<input placeholder="请输入" name="Phone" v-model="inputtext1.Phone" @blur="verifyTP(inputtext1.Phone)"/> 
							</li>
							<li class="items" >
								<span>开户行</span>
								<input placeholder="请输入" name="BankName" v-model="inputtext1.BankName"  @blur="verifyB(inputtext1.BankName)" /> 
								<i class="gIcon" @touchend.stop.prevent="goCompany('1',inputtext1,'bank')"></i>
							</li>
							<li class="items"  >
								<span>账号</span>
								<input placeholder="请输入" name="BankAccount" v-model="inputtext1.BankAccount" @blur="verifyZ(inputtext1.BankAccount)"/>
							</li>		
						<div class="orderBtn"><input type="submit"  value="确定"></div>   
					</form>
				</ul>
			...
			<div class="generalWtap"  v-show="2 == showIndex">
				<ul class="general">
					   <form @submit.prevent="submitFn2">  
							<li class="items" >
								<span>个人姓名</span>
								<input placeholder="请输入" name="Name" v-model="inputtext2.Name"   @blur="verifyPer(inputtext2.Name)"/>
							</li>	
						<div class="orderBtn"><input type="submit"  value="确定"></div>   
					</form>
				</ul>
...
export default {
  data() {
    return { 
      ...
      showHtml:false, 	
      title:"设置发票信息",
      billType:["增值税普通-电子票","增值税专用-电子票","个人-电子票"],
