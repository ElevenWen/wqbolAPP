...
<template>
  <div id="mine" >
    ...
    <div  v-show="showHtml">
        ...
        <div :class="{orderName:true,orderNameMore:cusDeliveryData.Address.length<15?true:false}">{{cusDeliveryData.ConsigneeName}}    {{cusDeliveryData.ConsigneeMobile}}
    ...
    <div class="mainWrap" v-show="showHtml">
      <div class="header">
        <div class="swiperWraps"  >
          <div  class="innerWrap">
            <ul class="swiperWrap">
                <li class="items"  v-for=" (items,index) in orderList.list" :key="index"> 
      ...
      <div class="discountsWrap">
        ...
        <div class="discountsB" v-if="coin.isUsed"> 
           <span>可用{{coin.Coin}}记账币抵扣{{coin.RMB}}元<img @click="ruleB_Show" height="0.5135135135135135rem " width="0.5135135135135135rem " src="../../assets/images/daiFu/explain@2x.png" alt=""></span>
           <span><mt-switch v-model="value" @change="coinChange"></mt-switch></span> 
        </div>
      </div>
      <div class="payMoneyWrap">
         <div class="payMoney"> 
           <span>商品金额</span><span  class="themeC productM">
           {{totalPrice | formatPrice |currency}}<b>{{totalPrice | formatPriceX}}</b></span>
        </div>
        <!-- 优惠不为0  -->
         <div class="tangibleMoney" v-if="couponShow"> 
          ...
          <div class="tangibleMoney" v-if="value"> 
           <span>记账币抵扣</span><span class="themeC">-{{coin.RMB | formatPrice |currency}}<b>{{coin.RMB | formatPriceX}}</b></span>
        </div>
      </div>
    </div>
    <!-- 固定地步bar -->
    ...
    <div class="fixBottomWrap"  v-show="showHtml">
      <div >
        ...
        <div class="moneyWrap">
          ...
          <span class="themeC productM"  v-cloak > 
            {{actualPay | formatPrice |currency}}<b>{{actualPay | formatPriceX}}</b>
          </span>
        </div>
...
import { Indicator ,MessageBox ,Toast} from 'mint-ui';
    ...
    this.getBalance_comfirmorder().then(()=>{
     ...
     // this.discountPriceFn();//刷新页面需要充值优惠券， 此代码不在调用
      ...
      this.GetCusInvoceAndCoupon(IC_params).then(res=>{
         this.couponData  = `你有${this.couponNum}张优惠券可用，去选择`;
         this.couponC = false;
          //用户没有选择优惠券重新 存入本地
          var couponObj = {
            "price":"-10.00",
  ...
  methods:{
    ...
    imidiateOrder(){
        ...
        //  收货地址
         ...
         let couponIdList =  tool.loadFromLocal("_SETBILL","cCoupon","couponObj",false);
          ...
          // 默认合同需要的客户id
          let cusId = this.commonTool.loadFromLocal("login_user_id","login_user_id","login_user_id").Id;
                ...
                if(this.orderId){
                      if(this.actualPay=='0.00'){
                        ...
                        this.successShow = true;
                         ...
                         this.$router.push({'name':'payMethod',query:{orderNum:this.orderId,actPrice:this.actualPay,bookkeepCurrency :this.coin.RMB,customerId:cusId}}) 
             ...
             if(paramsOrder.couponId == 'undefined'){
              ...
              this.imidiateFn(paramsOrder).then((res)=>{
                ...
                if(this.orderId){ 
                    ...
                    this.$router.push({'name':'payMethod',query:{orderNum:this.orderId,actPrice:this.actualPay,bookkeepCurrency :this.coin.RMB,isOwn:'false',customerId:cusId}}) 
                  }
                }    
                return
              },()=>{
    ...
    recentCompany(){
      ...
      if(this.isSelected.length >= 2 && this.person.length >= 2 && this.personTel.length == 11){
          this.requestInfo();
      }else{
          Toast({
              message: '信息错误',
